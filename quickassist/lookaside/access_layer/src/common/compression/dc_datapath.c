                                       ...
                                       CpaDcRqResults *pDcResults)
{
    ...
    CpaBoolean verifyHwIntegrityCrcs = pCookie->verifyHwIntegrityCrcs;
                                           ...
                                           CpaDcRqResults *pDcResults)
{
    ...
    /* Compare H/W CRCs against software ones if required */
    if (CPA_TRUE == pCookie->verifyHwIntegrityCrcs)
    {
        /* Calculate checksum on input data */
...
void dcCompression_ProcessCallback(void *pRespMsg)
{
    ...
    if ((CPA_TRUE == cmpPass) && (CPA_TRUE == xlatPass))
    {
        ...
        {
#ifdef _WIN64
            COMPRESSION_TIMESTAMP_END(pService, pCookie);
#endif
            if (DC_COMPRESSION_REQUEST == compDecomp)
            {
...
#ifdef _WIN64
#ifdef COUNTERS
                /*For EXTENDED_WINDOWS_COUNTERS
                  Add latency of request to sum of latencies for compress*/
                COMPRESSION_STAT_ADD2(
                    sumCompRequestLatency,
                    ((dc_compression_cookie_t *)pCookie)->timeStamp.QuadPart,
                    pService,
                    DcStats2);
                COMPRESSION_STAT_ADD2(numCompBytesThroughput,
                                      pResults->consumed,
                                      pService,
                                      DcStats2);
#endif
...
#ifdef COUNTERS
                /*For EXTENDED_WINDOWS_COUNTERS
                  Add latency of request to sum o latencies for compress*/
                COMPRESSION_STAT_ADD2(
                    sumDecompRequestLatency,
                    ((dc_compression_cookie_t *)pCookie)->timeStamp.QuadPart,
                    pService,
                    DcStats2);
                COMPRESSION_STAT_ADD2(numDecompBytesThroughput,
                                      pResults->produced,
                                      pService,
                                      DcStats2);
#endif
...
#ifdef ICP_PARAM_CHECK
STATIC CpaStatus dcParamCheck(CpaInstanceHandle dcInstance,
                              CpaDcSessionHandle pSessionHandle,
                              sal_compression_service_t *pService,
                              CpaBufferList *pSrcBuff,
                              CpaBufferList *pDestBuff,
...
CpaStatus dcDecompressDataCheck(CpaInstanceHandle insHandle,
                                CpaDcSessionHandle pSessionHandle,
                                CpaBufferList *pSrcBuff,
                                CpaBufferList *pDestBuff,
                                CpaDcRqResults *pResults,
