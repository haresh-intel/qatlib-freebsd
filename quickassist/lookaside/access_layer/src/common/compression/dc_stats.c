...
CpaStatus dcStatsInit(sal_compression_service_t *pService)
{
    ...
    pService->pCompStatsArr =
        LAC_OS_MALLOC(COMPRESSION_NUM_STATS * sizeof(OsalAtomic));
...
/* same functionality as CpaGetStats() but for the larger DcStats structure */
CpaStatus dcGetStats2(CpaInstanceHandle dcInstance, DcStats2 *pStatistics)
{
    sal_compression_service_t *pService = NULL;
    CpaInstanceHandle insHandle = NULL;
    ...
    /* Retrieves the statistics for compression */
    COMPRESSION_STATS_GET2(pStatistics, pService);
