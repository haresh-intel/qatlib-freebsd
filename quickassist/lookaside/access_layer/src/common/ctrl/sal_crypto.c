...
#ifdef _WIN64
    /* Short cut pointer to avoid traversing linked list at dispatch level */
    pCryptoService->generic_service_info.device = device;
#endif
                                   ...
                                   sal_service_t *service)
{
    ...
    if (CPA_TRUE == pStatsCollection->bStatsEnabled)
    {
        ...
        pCryptoService->debug_file = LAC_OS_MALLOC(sizeof(debug_file_info_t));
        if (NULL == pCryptoService->debug_file)
        {
            LAC_OS_FREE(instance_name);
            return CPA_STATUS_RESOURCE;
        ...
        status = icp_adf_debugAddFile(device, pCryptoService->debug_file);
        if (CPA_STATUS_SUCCESS != status)
        {
            LAC_OS_FREE(instance_name);
            LAC_OS_FREE(pCryptoService->debug_file);
                                 ...
                                 sal_service_t *service)
{
    ...
    /* Gen4 device has 2 rings per bank (1 ring pair), so only one type of
     * service can be assigned to the bank. Asym and sym services will be
...
#endif
#ifndef KERNEL_SPACE
    /* User instances only support poll mode */
    if (SAL_RESP_POLL_CFG_FILE != pCryptoService->isPolled)
    {
...
#ifdef _WIN64
    /* At this point there should not be any accesses to the device */
    pCryptoService->generic_service_info.device = NULL;
#endif
 ...
 ******************************************************************************
 * @ingroup cpaCyCommon
 * Crypto specific polling function which polls an asym crypto ring.
 *****************************************************************************/
                                 ...
                                 Cpa32U response_quota)
{
     ...
     * Populate our trans handle table with the appropriate handles.
     */
    trans_hndTable[TH_SINGLE_RX] = crypto_handle->trans_handle_asym_rx;
    /* Call adf to do the polling. */
    status = icp_adf_pollInstance(
                                            ...
                                            Cpa32U *numInflightRequests)
{
    ...
    return icp_adf_dp_getInflightRequests(crypto_handle->trans_handle_sym_tx,
                                          maxInflightRequests,
                                          numInflightRequests);
}
...
{
    sal_crypto_service_t *crypto_handle =
