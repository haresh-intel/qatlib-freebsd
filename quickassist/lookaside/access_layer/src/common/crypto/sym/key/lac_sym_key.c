...
const static Cpa8U key256[HKDF_SUB_LABEL_KEY_LENGTH] = {
    0, 16, 9, 't', 'l', 's', '1', '3', ' ', 'k', 'e', 'y', 0
};
const static Cpa8U key384[HKDF_SUB_LABEL_KEY_LENGTH] = {
    0, 32, 9, 't', 'l', 's', '1', '3', ' ', 'k', 'e', 'y', 0
};
const static Cpa8U keyChaChaPoly[HKDF_SUB_LABEL_KEY_LENGTH] = {
    0, 32, 9, 't', 'l', 's', '1', '3', ' ', 'k', 'e', 'y', 0
};
                                           ...
                                           icp_qat_fw_comn_flags cmnRespFlags)
{
    ...
    if (CPA_TRUE == respStatusOk)
    {
        LacKey_StatsInc(lacCmdId, LAC_KEY_COMPLETED, pCookie->instanceHandle);
    }
    else
    {
        status = CPA_STATUS_FAIL;
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        else if (ICP_QAT_FW_LA_CMD_TLS_V1_1_KEY_DERIVE == lacCmdId)
        {
                 ...
                 CPA_CY_KEY_TLS_OP_USER_DEFINED == pKeyGenTlsOpData->tlsOp))
            {
                 ...
                 * (secret length + 1)/2 will always give the ceil as
                 * division by 2
                 * (>>1) will give the smallest integral value not less than
                 * arg
                 */
                tlsPrefixLen =
                ...
                /* Compute SHA1 on the second half of the pre_master_secret
                 */
        ...
        else if (ICP_QAT_FW_LA_CMD_TLS_V1_2_KEY_DERIVE == lacCmdId)
        {
            ...
            if (CPA_TRUE == precompute)
            {
                 ...
                 * RFC 4868: For SHA-256 Block size is 512 bits, for SHA-384
                 * and SHA-512 Block size is 1024 bits Initialize pointer
                 ...
                 (ICP_QAT_FW_LA_CMD_HKDF_EXTRACT_AND_EXPAND_LABEL >= lacCmdId))
        {
            ...
            hashSetupData.digestResultLenInBytes =
                cipherSuiteHKDFHashSizes[hashAlgCipher][LAC_KEY_HKDF_DIGESTS];
        ...
        LacSymQat_HashContentDescInit(
            ...
            instanceHandle,
            &hashSetupData,
            pCookie->contentDesc, /* Pointer to base of hw setup block */
            LAC_SYM_KEY_NO_HASH_BLK_OFFSET_QW,
            ICP_QAT_FW_SLICE_DRAM_WR,
            qatHashMode,
            CPA_FALSE, /* Not using sym Constants Table in Shared SRAM */
                           ...
                           icp_qat_fw_la_cmd_id_t cmdId)
{
             ...
             cmdId <= ICP_QAT_FW_LA_CMD_HKDF_EXTRACT_AND_EXPAND_LABEL)
    {
        ...
        if (0 == (uOutputLen = getDigestSizeFromHashAlgo(hashAlgorithm)))
        {
        ...
        switch (cmdId)
        {
            ...
            case ICP_QAT_FW_LA_CMD_HKDF_EXTRACT_AND_EXPAND_LABEL: {
                ...
                for (i = 0; i < HKDF_Data->numLabels; i++)
                {
                    ...
                    if (maxLabelLen < HKDF_Data->label[i].labelLen)
                    {
                        LAC_INVALID_PARAM_LOG1(
                            "CpaCyKeyGenHKDFOpData.label[%d].labelLen", i);
                        return CPA_STATUS_INVALID_PARAM;
                    }

                    if (HKDF_Data->label[i].sublabelFlag & ~HKDF_SUB_LABELS_ALL)
                    {
                        LAC_INVALID_PARAM_LOG1(
                         ...
                         subl_number++)
                    {
                        ...
                        if (subl_mask & 1)
                        {
                            uOutputLen += cipherSuiteHKDFHashSizes[cipherSuite]
                                                                  [subl_number];
                        }
                        subl_mask >>= 1;
                    }
...
CpaStatus LacSymKey_Init(CpaInstanceHandle instanceHandle_in)
{
    ...
    pService->pLacKeyStats =
        LAC_OS_MALLOC(LAC_KEY_NUM_STATS * sizeof(OsalAtomic));
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        /* Copy the TLS v1.2 labels into the dynamically allocated structure */
