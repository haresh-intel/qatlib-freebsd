...
#define ASYM_TX_MSG_SIZE (LAC_QAT_ASYM_REQ_SZ_LW * LAC_LONG_WORD_IN_BYTES)

#define LAC_QAT_ASYM_RESP_SZ_LW 8
/**< @ingroup LacAsymCommon
 * LAC PKE QAT Response message size) */
#define ASYM_RX_MSG_SIZE (LAC_QAT_ASYM_RESP_SZ_LW * LAC_LONG_WORD_IN_BYTES)
...
#define LAC_CHECK_FLAT_BUFFER_PARAM_PKE(                                       \
    ...
    do                                                                         \
    {                                                                          \
        int type = (int)checkType;                                             \
        int len = (int)lenInBytes;                                             \
        LAC_CHECK_FLAT_BUFFER(pBuffer);                                        \
        if ((pBuffer)->dataLenInBytes == 0)                                    \
        {                                                                      \
            LAC_INVALID_PARAM_LOG(#pBuffer " has incorrect length of zero");   \
        ...
        if ((CHECK_EQUALS == type) && (len != LacPke_GetMinBytes(pBuffer)))    \
        {                                                                      \
            LAC_INVALID_PARAM_LOG(#pBuffer                                     \
                                  " contains number of incorrect size");       \
            return CPA_STATUS_INVALID_PARAM;                                   \
        }                                                                      \
        else if ((CHECK_LESS_EQUALS == type) &&                                \
                 (LacPke_GetMinBytes(pBuffer) > len))                          \
        {                                                                      \
            LAC_INVALID_PARAM_LOG(#pBuffer                                     \
                                  " contains number that is too large");       \
            return CPA_STATUS_INVALID_PARAM;                                   \
        }                                                                      \
        else if (len && (CHECK_GREATER_EQUALS == type) &&                      \
                 (LacPke_GetMinBytes(pBuffer) < len))                          \
        {                                                                      \
            LAC_INVALID_PARAM_LOG(#pBuffer                                     \
...
Cpa32S LacPke_Compare_Optimize(const CpaFlatBuffer *pFlatBufferA,
                               const CpaFlatBuffer *pFlatBufferB,
                               Cpa32S delta);
...
Cpa32S LacPke_CompareOne(const CpaFlatBuffer *pLargeInteger);

Cpa32S LacPke_CompareTwo(const CpaFlatBuffer *pLargeInteger);
/**
 *******************************************************************************
 * @ingroup LacAsymCommonUtils
 ...
 *pData of the FlatBuffer can be NULL iff dataLenInBytes=0
 ...
 *     This can be used to locate the position of the msb (position indexed from
 *0). For example, floor[log2(1)] = 0 = b1 (the 0th bit is set). For example,
 *floor[log2(8)] = 3 = b1000 (the 3rd bit is set). For example, floor[log2(21)]
 *= 4 = b10101 (the 4th bit is set).
 *
 ...
 *number in FlatBuffer. If dataLenInBytes=0 then this will be set to 0.
 ...
 *to dataLenInBytes-1. If dataLenInBytes=0 (i.e. pData == NULL) then index will
 *be set to -1
 ...
 *buffer. If more than 4096 bits are required we return _INVALID_PARAM (we have
 *no pke service for bigger numbers)
 *
 * @param[in]  pBuffer          pointer to FlatBuffer (should NOT be NULL).
 *pData of the FlatBuffer can be NULL iff dataLenInBytes=0
