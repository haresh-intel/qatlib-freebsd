...
CpaStatus LacRsa_Type2StdsCheck(CpaCyRsaPrivateKeyRep2 *pPrivateKeyRep2)
{
    ...
    if (LacPke_CompareOne(&(pPrivateKeyRep2->exponent1Dp)) <= 0)
    {
        LAC_INVALID_PARAM_LOG("exponent1Dp must be > 1");
        return CPA_STATUS_INVALID_PARAM;
    }
    if (LacPke_Compare_Optimize(&(pPrivateKeyRep2->exponent1Dp),
    ...
    if (LacPke_CompareOne(&(pPrivateKeyRep2->exponent2Dq)) <= 0)
    {
        LAC_INVALID_PARAM_LOG("exponent2Dq must be > 1");
        return CPA_STATUS_INVALID_PARAM;
    }
    if (LacPke_Compare_Optimize(&(pPrivateKeyRep2->exponent2Dq),
    ...
    if (LacPke_Compare_Optimize(&(pPrivateKeyRep2->coefficientQInv),
                                &(pPrivateKeyRep2->prime1P),
                                0) >= 0)
    {
