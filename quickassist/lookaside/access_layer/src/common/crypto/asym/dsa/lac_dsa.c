                            ...
                            CpaFlatBuffer *pG)
{
    ...
    {
        /* Check 1 < h < p-1 */
        if ((LacPke_CompareOne(&(pOpData->H)) <= 0) ||
            (LacPke_Compare_Optimize(&(pOpData->H), &(pOpData->P), 1) >= 0))
        {
                            ...
                            CpaFlatBuffer *pY)
{
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        /* Check 1 < g < p */
        if ((LacPke_CompareOne(&(pOpData->G)) <= 0) ||
            (LacPke_Compare_Optimize(&(pOpData->G), &(pOpData->P), 0) >= 0))
                        ...
                        CpaFlatBuffer *pR)
{
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        if ((LacPke_CompareZero(&(pOpData->K), 0) == 0) ||
            (LacPke_Compare_Optimize(&(pOpData->K), &(pOpData->Q), 0) >= 0))
        {
                        ...
                        CpaFlatBuffer *pS)
{
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        if ((LacPke_CompareZero(&(pOpData->R), 0) == 0) ||
            (LacPke_Compare_Optimize(&(pOpData->R), &(pOpData->Q), 0) >= 0))
        {
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        if ((LacPke_CompareZero(&(pOpData->X), 0) == 0) ||
            (LacPke_Compare_Optimize(&(pOpData->X), &(pOpData->Q), 0) >= 0))
        {
            LAC_INVALID_PARAM_LOG("X out of Range");
            status = CPA_STATUS_INVALID_PARAM;
        }
                         ...
                         CpaBoolean *pVerifyStatus)
{
    ...
    {
        /* Check y < p */
        if (LacPke_Compare_Optimize(&(pOpData->Y), &(pOpData->P), 0) >= 0)
        {
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        if ((LacPke_CompareZero(&(pOpData->S), 0) == 0) ||
            (LacPke_Compare_Optimize(&(pOpData->S), &(pOpData->Q), 0) >= 0))
        {
...
CpaStatus LacDsa_Init(CpaInstanceHandle instanceHandle)
{
    ...
    pCryptoService->pLacDsaStatsArr =
        LAC_OS_MALLOC(LAC_DSA_NUM_STATS * sizeof(OsalAtomic));
