...
/*
 * Optimized function for LacPke_Compare for IA offload cost
 */
                               ...
                               Cpa32S delta)
{
    ...
    if (lenB >= lenA)
    {
        /* short cut if flatBufferB >> flatBufferA */
        for (i = 0; i < lenB - lenA; i++)
        {
            if (pFlatBufferB->pData[i])
                return -1;
        }
        /* short cut when delta is 0 */
        if (delta == 0)
        {
            for (i = 0; i < lenA; i++)
            {
                if (pFlatBufferA->pData[i] ^
                    pFlatBufferB->pData[i + lenB - lenA])
                    return pFlatBufferA->pData[i] >
                                   pFlatBufferB->pData[i + lenB - lenA]
                               ? 1
                               : -1;
            }
    ...
    {
        /* short cut if flatBufferA >> flatBufferB */
        for (i = 0; i < lenA - lenB; i++)
        {
            if (pFlatBufferA->pData[i])
                return 1;
        }
        /* short cut wher delta is 0 */
        if (delta == 0)
        {
            for (i = 0; i < lenB; i++)
            {
                if (pFlatBufferB->pData[i] ^
                    pFlatBufferA->pData[i + lenA - lenB])
                    return pFlatBufferB->pData[i] >
                                   pFlatBufferA->pData[i + lenA - lenB]
                               ? -1
                               : 1;
            }
    ...
    for (i = lenMin; i > 0; i--)
    {
        ...
        /*
         * Once get hit in notZero, we can break the cycle
         * and we know that the integers do not equal. However we
         * still do not know which number is greater.
         */
        notZero |= (delta & 0xFF);
        /* signed arithmetic shift */
        delta >>= LAC_NUM_BITS_IN_BYTE;
...
Cpa32S LacPke_CompareOne(const CpaFlatBuffer *pLargeInteger)
{
    ...
    return pLargeInteger->pData[(pLargeInteger->dataLenInBytes - 1)] - 1;
}
...
Cpa32S LacPke_CompareTwo(const CpaFlatBuffer *pLargeInteger)
{
    ...
    return pLargeInteger->pData[(pLargeInteger->dataLenInBytes - 1)] - 2;
}
