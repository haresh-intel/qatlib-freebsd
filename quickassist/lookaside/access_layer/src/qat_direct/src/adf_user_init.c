...
/* Sleeping time before subsystem is started. */
#define SLEEP_TIME 50000
/* Loop times before subsystem is started. */
#define SLEEP_LOOPS 10000000
Cpa32U userStartSleepTime = SLEEP_TIME;
Cpa32U userStartSleepLoops = SLEEP_LOOPS;
    ...
    subservice_registation_handle_t *subsystem)
{
    ...
    {
        if (CPA_STATUS_SUCCESS != ICP_MUTEX_INIT(&subsystemTableLock))
        {
            ADF_ERROR("Failed to initialize table lock mutex\n");
            return CPA_STATUS_FAIL;
        }
        set_sleep_time(SLEEP_TIME, SLEEP_LOOPS);
    }

    if (CPA_STATUS_SUCCESS != ICP_MUTEX_LOCK(&subsystemTableLock))
    {
        ADF_ERROR("Failed to lock table mutex\n");
    ...
    if (CPA_STATUS_SUCCESS == status)
    {
        ...
        if (CPA_STATUS_SUCCESS != ICP_MUTEX_UNLOCK(&subsystemTableLock))
        {
            ADF_ERROR("Failed to unlock table mutex\n");
        }
        return CPA_STATUS_FAIL;
    }
    ...
    subservice_registation_handle_t *subsystem)
{
    ...
    {
        if (CPA_STATUS_SUCCESS != ICP_MUTEX_UNINIT(&subsystemTableLock))
        {
            ADF_ERROR("Failed to uninitialize table mutex\n");
        }
    }
    return CPA_STATUS_SUCCESS;
}
    ...
    subservice_registation_handle_t *subsystem_hdl)
{
    ...
    for (i = 0; i < ADF_MAX_DEVICES; i++)
    {
        ...
        {
            /*
             * If the device is started in the system need to wait till
             * the subsystem is started for this device or ...
             */
            while (!BIT_IS_SET(
