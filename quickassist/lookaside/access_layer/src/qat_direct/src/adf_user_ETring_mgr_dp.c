 ...
 * Data plane support function - Retrieves the inflight requests from handle
 */
CpaStatus icp_adf_dp_getInflightRequests(icp_comms_trans_handle trans_hnd,
...
{
    adf_dev_ring_handle_t *ring = (adf_dev_ring_handle_t *)trans_hnd;
    ...
    /* Achieve the same things as adf_user_flush_requests but without locks */
                            ...
                            Cpa32U response_quota)
{
    ...
    if (msg_counter > 0)
    {
        *pRingHandle->in_flight -= msg_counter;
        ...
        if (msg_counter > pRingHandle->coal_resps_write_count)
        {
            pRingHandle->coal_resps_write_count =
        ...
        else
        {
            ...
            pRingHandle->coal_resps_write_count -= msg_counter;
        }
    }
    else
    {
