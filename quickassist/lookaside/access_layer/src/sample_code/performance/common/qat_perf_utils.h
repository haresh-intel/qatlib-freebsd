...
#include <sys/proc.h>
#include <sys/sched.h>
#define QAT_NANOSLEEP_DURATION 10
#ifndef AVOID_SOFTLOCKUP
/* For small dc payloads, huge number of retries are seen resulting in timeouts.
 * Hence slow down the request submissions by using sleep of 10 nanoseconds.
 */
#define AVOID_SOFTLOCKUP                                                       \
    do                                                                         \
    {                                                                          \
        if (sleepOnRetry)                                                      \
            sleepNano(QAT_NANOSLEEP_DURATION);                                                     \
        else                                                                   \
